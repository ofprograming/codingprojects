...
class handTracker():
    def __init__(self, mode = False, maxHands = 2, detectionCon = 0.5, modeComplexity = 1, trackCon = 0.5 ):
        ...
        def HandsFinder(self, image, draw= True):
            ...
            self.results = self.hands.proccess(imageRGB)
            if self.results.multi_hand_landmarks: 
                for handLms in self.results.multi_hand_landmarks: 
                    if draw: 
                        self.mpDraw_landmarks(image, handLms, self.mpHands.HAND_CONNECTIONS)
                    return image 
        def positionFinder(self, image, handsNo = 0, draw = True ):
            lmlist = []
            if self.results.multi_hand_landmarks: 
